...
# アイテム設計バイブル (Items & Balance Design) v2.5
...
### 2.3 Key Items (重要アイテム) - v2.7 Update
イベントトリガーやパッシブ効果を持つ非売品。これらは「知識」ではなく「試験攻略情報」である。

*   **USBメモリ**: 「60点の壁」を超えるための必須アイテム。
    *   *効果*: 成功時 +20点 / 失敗時 -15 SAN, -10 HP
    *   *成功率*: アルゴリズム知識依存 (Min 30% -> Max 95%)。60点未満では博打要素が強く、失敗時の物理ダメージ（ウィルス感染）も追加された。
*   **検証済み過去問**: 上位互換。成功率98%。
    *   *特徴*: ほぼ確実にスコアアップするが、入手には高い人脈が必要。
...
*   推奨レンジ: 1.1倍 〜 2.5倍
*   **v2.7**: 総合演習（Study All）においては、効果係数が50%減衰される仕様に変更。
...
*   **v2.7**: USB解析のリスク強化（HPダメ追加）と総合演習のバフ減衰を定義。
