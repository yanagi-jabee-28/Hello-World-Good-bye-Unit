...
| 方法 | ターン効率 | リスク | 適用場面 | 教材バフ |
|------|-----------|--------|----------|----------|
| **個別学習** | 低 (1科目) | なし | 特定科目の弱点克服 | 100% |
| **総合演習** | 高 (全科目) | なし | 時間がないときの保険 | **50%減衰** |
| **USB解析** | 超高 | 高 (アルゴ依存) | アルゴ60+での時短 | なし |
| **過去問** | 超高 | 低 (98%) | 確実な追い込み | なし |
| **参考書** | 中 (持続) | なし | 安定した基礎固め | 補助 |

**設計意図**:
- 総合演習は「浅く広く」を体現するため、サプリ等の学習効率バフを半減適用する。
- USB解析は失敗時に精神だけでなく肉体ダメージ（HP減少）も与えることで、安易な使用を牽制する。
- 人脈ボーナスは「なんとなく仲が良い」ではなく、Tier制の閾値（40/60/80）を超えることで明確な恩恵が得られるようにする。
